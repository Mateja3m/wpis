...
function validateAsset(asset: PaymentAsset): void {
  ...
  if (asset.type === "erc20") {
    if (!asset.contractAddress) {
      throw new Error("asset.contractAddress is required for erc20 assets");
    }
    if (!isValidEvmAddress(asset.contractAddress)) {
      throw new Error("asset.contractAddress must be a valid EVM address");
    }
  }
}
...
export function validateCreateIntentInput(input: CreateIntentInput): void {
  ...
  if (Number.isNaN(expiresAt.getTime())) {
    throw new Error("expiresAt must be a valid ISO datetime string");
  }
  ...
  if (input.confirmationPolicy && input.confirmationPolicy.minConfirmations < 0) {
    throw new Error("confirmationPolicy.minConfirmations must be >= 0");
  }
}
